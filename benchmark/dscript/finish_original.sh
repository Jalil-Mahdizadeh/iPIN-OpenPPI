...
set -euo pipefail
umask 077
dscript_root=${DSCRIPT_BENCHMARK_ROOT:?Set by submit_original.sh}
bench_root=$(dirname -- "$dscript_root")
repo_root=$(dirname -- "$bench_root")
private="$dscript_root/private/original-v1"
...
"${base[@]}" --bind "$private/session:/session:ro" --bind "$private/shards:/shards:ro" \
  ...
  python /code/gpu_guard.py /code/score_shard.py merge --workers 4 > "$dscript_root/logs/original-merge.log" 2>&1
...
"${base[@]}" --bind "$private/session:/session:ro" --bind "$private/predictions:/predictions:ro" \
  ...
  > "$dscript_root/logs/original-reference-import.log" 2>&1
mkdir "$private/freeze"
"${base[@]}" --bind "$private/session:/session:ro" --bind "$private/predictions:/predictions:ro" \
  --bind "$private/references:/references:ro" --bind "$private/freeze:/output:rw" \
  "$image" "${runtime[@]}" "${entry[@]}" freeze > "$dscript_root/logs/original-prediction-freeze.log" 2>&1
...
"${base[@]}" --bind "$private/session:/session:ro" --bind "$private/predictions:/predictions:ro" \
  ...
  > "$dscript_root/logs/original-evaluate.log" 2>&1
mkdir -p "$dscript_root/results"
mkdir "$dscript_root/results/original-v1"
"${base[@]}" --bind "$private/evaluation/RESULTS.json:/aggregate/RESULTS.json:ro" \
  --bind "$dscript_root/results/original-v1:/output:rw" "$image" "${runtime[@]}" "${entry[@]}" publish \
  > "$dscript_root/logs/original-publish.log" 2>&1

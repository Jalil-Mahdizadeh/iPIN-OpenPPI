...
output="$dscript_root/private/original-v1/shards/$rank_name"
mkdir -p "$output/tmp"
...
printf 'GPU_BINDING rank=%s uuid=%s CUDA_VISIBLE_DEVICES=%s\n' \
  "$rank" "${dscript_gpu_uuids[0]}" "${CUDA_VISIBLE_DEVICES:?Slurm must assign a GPU}"
...
exec env -u APPTAINER_BIND -u APPTAINER_BINDPATH -u SINGULARITY_BIND -u SINGULARITY_BINDPATH \
  ...
  --bind "$dscript_root/runs/original-v1/scorer_bundle:/bundle:ro" \
  --bind "$dscript_root/runs/original-v1/scorer_bundle/code:/code:ro" \
  --bind "$dscript_root/private/original-v1/session:/session:ro" \

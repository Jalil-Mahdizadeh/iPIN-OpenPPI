...
mkdir -p "$dscript_root/runs/retrained-v1/data" "$dscript_root/runs/retrained-v1/tmp" "$dscript_root/logs"
...
exec env -u APPTAINER_BIND -u APPTAINER_BINDPATH -u SINGULARITY_BIND -u SINGULARITY_BINDPATH \
  ...
  --bind "$dscript_root/scripts/retrained_v1:/code:ro" \
  --bind "$bench_root/tuna/data:/source_data:ro" \
  --bind "$dscript_root/runs/retrained-v1/data:/data:ro" \
  --bind "$dscript_root/runs/retrained-v1:/output:rw" --pwd /output \

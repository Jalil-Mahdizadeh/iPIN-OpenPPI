...
def prediction_values(tokens,table):
    if table.column_names==['candidate_token','dscript_original']:
...
def score():
    ...
    for cell in CELLS:
        ...
        if symmetry[cell]>read('/bundle/SCORER_FREEZE.json')['logit_tolerance']:
...
def import_references():
    ...
    original=read('/original_dscript/PREDICTIONS.json')
    scorer=bundle_check()
    assert sha('/original_dscript/PREDICTIONS.json')==scorer['original_predictions_manifest_sha256']
    assert original['complete_unique_finite_coverage'] and not original['truth_accessed']
    verify('/original_dscript',original['files'])
    (output/'dscript_original').mkdir(exist_ok=False)
    for item in original['files']:
        source=Path('/original_dscript')/item['path'];target=output/'dscript_original'/source.name
        tokens=pq.read_table(cell_path('/session',item['cell']),columns=['candidate_token'])['candidate_token']
        prediction_values(tokens,pq.read_table(source));shutil.copyfile(source,target)
        files.append({**relative_record(target,output),'cell':item['cell'],'model':'dscript_original','historical_sha256':item['sha256']})
...
def evaluate():
    ...
    write(output/'EVALUATION_RESERVATION.json',{'at_utc':now(),'execution_id':'dscript_retrained_benchmark_v1',
    ...
    with tempfile.TemporaryDirectory(prefix='truth-',dir=output) as temporary:
        ...
        for cell in CELLS:
            ...
            if abs(points[ALL_SCORERS.index('dscript_original')]-original['cells'][cell]['metrics']['dscript_original']['concordance'])>1e-12:
                raise RuntimeError('Original D-SCRIPT reference metric did not reproduce')
            cell_result={}
            ...
            contrasts=[('dscript_retrained',reference) for reference in REFERENCES]
            contrasts.extend([('dscript_original','ipin_baseline'),('dscript_original','ipin_optimized')])
            cell_differences=[]
    ...
    write(output/'RESULTS.json',{'at_utc':now(),'execution_id':'dscript_retrained_benchmark_v1','cells':results,'differences':differences,
        'metric_qualification':qualification,'prediction_freeze_sha256':sha('/freeze/PREDICTION_FREEZE.json'),
        'original_model':'human_v1','retrained_model':'PU-D-SCRIPT; three fresh PPI heads, frozen native LM',
        'selected_epoch':read('/bundle/SCORER_FREEZE.json')['selected_epoch'],'elapsed_seconds':time.monotonic()-started,
        'historical_reference_points_reproduced':True,'historical_component_draws_reproduced':True,
        'original_dscript_point_reproduced':True,'U_is_not_negative':True,'test_previously_examined':True,
        'original_external_training_exposure':'Documented public-training sequence overlap; original is not a TRAIN-only comparison',
        'retrained_initialization':'Random PPI head; no human_v1 PPI weights used; native encoder remains externally pretrained',
...
def publish():
    ...
    lines=['# D-SCRIPT original and matched-data retrained benchmark results','',
        f"Completed: {result['at_utc']}. Original human_v1 and three-seed PU-D-SCRIPT, selected epoch {result['selected_epoch']}.",'',
        '| Model | C1 | C2 | C3 |','|---|---:|---:|---:|']
    for name in ('dscript_original','dscript_retrained','ipin_baseline','ipin_optimized',*SCORERS[1:]):
        values=[result['cells'][cell]['metrics'][name]['concordance'] for cell in CELLS]
        lines.append('| '+name+' | '+' | '.join(f'{x:.6f}' for x in values)+' |')
    lines.extend(['','Primary contrast: retrained PU-D-SCRIPT ensemble minus optimized iPIN on C3. See paired_differences.csv for paired intervals, including retrained versus original; C1/C2 are secondary.',

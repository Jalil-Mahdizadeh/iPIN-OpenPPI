...
def evaluate():
    ...
    write(output/'EVALUATION_RESERVATION.json',{'at_utc':now(),'execution_id':'plm_interact_original_benchmark_v1',
    ...
    write(output/'RESULTS.json',{'at_utc':now(),'execution_id':'plm_interact_original_benchmark_v1','cells':results,'differences':differences,
        'metric_qualification':qualification,'prediction_freeze_sha256':sha('/freeze/PREDICTION_FREEZE.json'),
        'original_model':'PLM-interact-650M-humanV11','elapsed_seconds':time.monotonic()-started,
...
def publish():
    ...
    lines=['# Original PLM-interact benchmark results','',f"Completed: {result['at_utc']}. Original 650M humanV11 checkpoint; no retraining.",'',
        '| Model | C1 | C2 | C3 |','|---|---:|---:|---:|']
    ...
    lines.extend(['','Primary contrast: original PLM-interact minus optimized iPIN on C3. See paired_differences.csv for intervals; C1/C2 are secondary.',

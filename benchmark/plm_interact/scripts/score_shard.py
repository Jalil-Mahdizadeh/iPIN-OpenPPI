...
def work(rank,workers):
    ...
    for cell in CELLS:
        ...
        truncated=int(np.sum(scorer.lengths[a]+scorer.lengths[b]+3>frozen['maximum_tokens']))
        ...
        while done<len(subset):
            end=min(done+512,len(subset));values[done:end]=scorer.scores(a[done:end],b[done:end])[:,0];values.flush()
            ...
            if done%2048==0 or done==len(subset):print(info,flush=True)

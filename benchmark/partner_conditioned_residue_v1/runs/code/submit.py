...
def main():
    ...
    previous = None
    try:
        with (ROOT / 'logs/interactive-mean_pool.log').open('x') as log:
            process = subprocess.Popen(['bash', str(ROOT / 'run.sh'), 'python', '/code/gpu_guard.py',
                                        '/code/train.py', '--recipe', 'mean_pool'],
                                       stdin=subprocess.DEVNULL, stdout=log, stderr=subprocess.STDOUT,
                                       start_new_session=True, env=env)
        value['interactive_control'] = {'pid': process.pid, 'slurm_allocation': os.environ.get('SLURM_JOB_ID'),
                                        'log': 'logs/interactive-mean_pool.log'}
        persist()

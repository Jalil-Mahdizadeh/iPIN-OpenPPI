...
def main():
    ...
    locks = Path('/output/locks'); locks.mkdir(exist_ok=True)
    with (locks / (recipe['name'] + '.lock')).open('a') as handle:
        print({'recipe': recipe['name'], 'event': 'waiting_for_exclusive_recipe_lock'}, flush=True)
        fcntl.flock(handle.fileno(), fcntl.LOCK_EX)
        print({'recipe': recipe['name'], 'event': 'recipe_lock_acquired'}, flush=True)
        device = cuda()

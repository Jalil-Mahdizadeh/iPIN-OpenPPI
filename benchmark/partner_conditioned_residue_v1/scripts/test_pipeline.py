...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='pipeline-', dir='/output/.tmp') as directory:
        ...
        with (root / 'recipe.lock').open('a') as first, (root / 'recipe.lock').open('a') as second:
            ...
            try:
                ...
                raise AssertionError('A second recipe worker acquired an occupied lock')
    ...
    result = {'at_utc': now(), 'passed': True, 'frozen_roundtrip_max_error': error,
              ...
              'recipe_lock_exclusion_and_release': True,

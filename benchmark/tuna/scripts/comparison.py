...
def freeze_predictions():
    ...
    for cell in CELLS:
        ...
        if not np.isfinite(values).all() or not np.array_equal(values[:,1],values[:,2:].mean(1,dtype=np.float64)):
...
def evaluate():
    ...
    write(output/'EVALUATION_RESERVATION.json',{'at_utc':now(),'execution_id':'tuna_pu_benchmark_v1',
    ...
    with tempfile.TemporaryDirectory(prefix='truth-',dir=output) as temporary:
        ...
        for cell in CELLS:
            ...
            contrasts.append(('tuna_retrained_ensemble','tuna_original'))
            cell_differences=[]
    ...
    write(output/'RESULTS.json',{'at_utc':now(),'execution_id':'tuna_pu_benchmark_v1','cells':results,'differences':differences,
...
def publish():
    ...
    lines=['# TUnA final benchmark results','',f"Completed: {result['at_utc']}. Retrained ensemble: epoch {result['selected_epoch']}.",'',
        '| Model | C1 | C2 | C3 |','|---|---:|---:|---:|']
    for name in ('tuna_original','tuna_retrained_ensemble',*REFERENCES):
        values=[result['cells'][cell]['metrics'][name]['concordance'] for cell in CELLS]
        lines.append('| '+name+' | '+' | '.join(f'{x:.6f}' for x in values)+' |')
    lines.extend(['','Primary contrast: retrained PU-TUnA ensemble minus optimized iPIN on C3. See paired_differences.csv for intervals; C1/C2 are secondary.',

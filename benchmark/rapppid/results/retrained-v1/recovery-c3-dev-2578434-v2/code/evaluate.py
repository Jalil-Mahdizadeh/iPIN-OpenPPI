...
INDEPENDENT_METRIC_TOLERANCE = 1e-9
...
def snapshot():
    ...
    value = {'at_utc': now(), 'execution_id': 'rapppid_recovery_c3_dev_2578434_v2',
             ...
             'independent_metric_tolerance': INDEPENDENT_METRIC_TOLERANCE,
             'attempt_disclosure': 'First attempt stopped before latest-checkpoint scoring: equivalent full-panel metric algorithms differed by 1.4264e-11 on the existing epoch-4 result. Verification tolerance is 1e-9 for million-row FP64 summation order; the historical scoring function is unchanged.',
...
def metric_checked(values, dev):
    ...
    # Also reverse the summation: count P above each U, then sum U weights.
    # Long-double accumulation avoids a million-element FP64 prefix sum here.
    positives = np.sort(values[dev['positive']])
    unlabeled = values[~dev['positive']]
    left = np.searchsorted(positives, unlabeled, side='left')
    right = np.searchsorted(positives, unlabeled, side='right')
    wins = len(positives) - (left + right) * .5
    weights = dev['weight'][~dev['positive']].astype(np.longdouble)
    reverse = float(np.sum(weights * wins, dtype=np.longdouble) / np.sum(weights, dtype=np.longdouble) / len(positives))
    error = max(abs(point - check), abs(point - reverse), abs(check - reverse))
    # Different monotone accumulation orders over 1,000,000 nonuniform weights
    # need a roundoff allowance; this does not change any reported score.
    assert error <= INDEPENDENT_METRIC_TOLERANCE, (point, check, reverse)
    return point, error
...
def evaluate():
    ...
    assert freeze['independent_metric_tolerance'] == INDEPENDENT_METRIC_TOLERANCE

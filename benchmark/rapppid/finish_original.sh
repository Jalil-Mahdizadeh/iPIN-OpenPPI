...
set -euo pipefail
umask 077
rapppid_root=${RAPPPID_BENCHMARK_ROOT:?Set by scripts/run_original.py}
bench_root=$(dirname -- "$rapppid_root")
repo_root=$(dirname -- "$bench_root")
private="$rapppid_root/private/original-v1"
...
image="$bench_root/containers/images/rapppid-native-arm64-v1.sif"
...
"${base[@]}" --bind "$private/session:/session:ro" --bind "$private/shards:/shards:ro" \
  ...
  python /code/gpu_guard.py /code/score_shard.py merge --workers 1 > "$rapppid_root/logs/original-merge.log" 2>&1
...
"${base[@]}" --bind "$private/session:/session:ro" --bind "$private/predictions:/predictions:ro" \
  ...
  > "$rapppid_root/logs/original-reference-import.log" 2>&1
mkdir "$private/freeze"
"${base[@]}" --bind "$private/session:/session:ro" --bind "$private/predictions:/predictions:ro" \
  --bind "$private/references:/references:ro" --bind "$private/freeze:/output:rw" \
  "$image" "${runtime[@]}" "${entry[@]}" freeze > "$rapppid_root/logs/original-prediction-freeze.log" 2>&1
...
"${base[@]}" --bind "$private/session:/session:ro" --bind "$private/predictions:/predictions:ro" \
  ...
  > "$rapppid_root/logs/original-evaluate.log" 2>&1
mkdir -p "$rapppid_root/results"
mkdir "$rapppid_root/results/original-v1"
"${base[@]}" --bind "$private/evaluation/RESULTS.json:/aggregate/RESULTS.json:ro" \
  --bind "$rapppid_root/results/original-v1:/output:rw" "$image" "${runtime[@]}" "${entry[@]}" publish \
  > "$rapppid_root/logs/original-publish.log" 2>&1

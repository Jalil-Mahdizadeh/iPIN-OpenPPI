...
def evaluate():
    ...
    write(output/'EVALUATION_RESERVATION.json',{'at_utc':now(),'execution_id':'rapppid_original_benchmark_v1',
    ...
    with tempfile.TemporaryDirectory(prefix='truth-',dir=output) as temporary:
        ...
        for cell in CELLS:
            ...
            contrasts=[(candidate,reference) for candidate in SCORERS[:2] for reference in REFERENCES]
            cell_differences=[]
    ...
    write(output/'RESULTS.json',{'at_utc':now(),'execution_id':'rapppid_original_benchmark_v1','cells':results,'differences':differences,
        'metric_qualification':qualification,'prediction_freeze_sha256':sha('/freeze/PREDICTION_FREEZE.json'),
        'original_model':'RAPPPID-released-mult-red-dreamy','elapsed_seconds':time.monotonic()-started,
        'original_model_revision':read('/bundle/SCORER_FREEZE.json')['model_revision'],
        'original_checkpoint_sha256':read('/bundle/SCORER_FREEZE.json')['checkpoint_sha256'],
...
def publish():
    ...
    lines=['# Original RAPPPID benchmark results','',f"Completed: {result['at_utc']}. Authors’ released red-dreamy multiplicative-head checkpoint; no retraining.",'',
        '| Model | C1 | C2 | C3 |','|---|---:|---:|---:|']
    for name in (*SCORERS,*REFERENCES):
        values=[result['cells'][cell]['metrics'][name]['concordance'] for cell in CELLS]
        lines.append('| '+name+' | '+' | '.join(f'{x:.6f}' for x in values)+' |')
    lines.extend(['','Primary contrast: original RAPPPID minus optimized iPIN on C3. See paired_differences.csv for intervals; C1/C2 are secondary.',

...
prediction and byte-identical original RAPPPID and iPIN references have been frozen.
"""
...
def prediction_values(tokens,table):
    if table.column_names==['candidate_token','rapppid_original_released_mult']:
        table=table.rename_columns(['candidate_token','score'])
...
def import_references():
    ...
    files=[]
    assert sha('/rapppid_predictions.json') == read('/bundle/SCORER_FREEZE.json')['original_predictions_manifest_sha256']
    definitions=[('rapppid_original_released_mult','/original_rapppid','/rapppid_predictions.json',''),
                 ('ipin_baseline','/original_baseline','/baseline_freeze.json',BASELINE),
    ...
    write(output/'REFERENCES.json',{'at_utc':now(),'files':files,'byte_identical_to_historical_predictions':True,
        ...
        'original_rapppid_manifest_sha256':sha('/rapppid_predictions.json'),
...
def freeze_predictions():
    ...
    for cell in CELLS:
        ...
        if not np.isfinite(values).all() or not np.array_equal(values[:,0], values[:,1:].mean(1,dtype=np.float64)):
...
def independent_points(scores, mask, weights):
    # Reverse the historical summation: count P above each U, then sum U
    # weights in long-double precision. No relabeling of U is implied.
    result=[]
    w=weights[~mask].astype(np.longdouble)
    for column in range(scores.shape[1]):
        p=np.sort(scores[mask,column]); u=scores[~mask,column]
        left=np.searchsorted(p,u,side='left'); right=np.searchsorted(p,u,side='right')
        wins=len(p)-(left+right)*.5
        result.append(float(np.sum(w*wins,dtype=np.longdouble)/np.sum(w,dtype=np.longdouble)/len(p)))
...
def evaluate():
    ...
    write(output/'EVALUATION_RESERVATION.json',{'at_utc':now(),'execution_id':'rapppid_recovery_test_v1',
    ...
    old=read('/historical_results.json'); original=read('/original_results.json')
    assert sha('/original_results.json')==read('/bundle/SCORER_FREEZE.json')['original_results_sha256']
    results={}; differences={}; started=time.monotonic()
    with tempfile.TemporaryDirectory(prefix='truth-',dir=output) as temporary:
        ...
        for cell in CELLS:
            ...
            independent_error=float(np.max(np.abs(points-independent_points(scores,mask,weights))))
            if independent_error>1e-9:
                raise RuntimeError('Independent full-panel metric check failed')
            ...
            original_expected=original['cells'][cell]['metrics']['rapppid_original_released_mult']['concordance']
            if abs(points[ALL_SCORERS.index('rapppid_original_released_mult')]-original_expected)>1e-12:
                raise RuntimeError('Original RAPPPID point failed reproduction')
            if metadata!=old['cells'][cell]['bootstrap'] or metadata!=original['cells'][cell]['bootstrap']:
            ...
            for name in REFERENCES:
                expected=original['cells'][cell]['metrics'][name]
                actual=cell_result[name]
                if actual['finite_bootstrap_draws']!=expected['finite_bootstrap_draws'] or not np.allclose(actual['percentile_95'],expected['percentile_95'],atol=1e-12,rtol=0):
                    raise RuntimeError('Historical reference interval failed reproduction')
            ...
            results[cell]={'positive_pairs':len(p),'unlabeled_pairs':len(u),'metrics':cell_result,'bootstrap':metadata,
                          'independent_metric_maximum_error':independent_error,'independent_metric_tolerance':1e-9}
    ...
    write(output/'RESULTS.json',{'at_utc':now(),'execution_id':'rapppid_recovery_test_v1','cells':results,'differences':differences,
        'metric_qualification':qualification,'prediction_freeze_sha256':sha('/freeze/PREDICTION_FREEZE.json'),
        'candidate_model':'PU-RAPPPID-mult: user-selected mid-epoch-8 recovery states','elapsed_seconds':time.monotonic()-started,
        'model_revision':read('/bundle/SCORER_FREEZE.json')['model_revision'],
        'selection':read('/bundle/SELECTION.json'),
        'completed_epoch8_claimed':False,'training_resumed':False,
        'original_reference_points_reproduced':True,
        ...
        'U_is_not_negative':True,'test_previously_examined':True,'original_external_training_exposure':'possible; not a TRAIN-only comparison',
...
def publish():
    ...
    with (output/'coverage.csv').open('x',newline='') as f:
        writer=csv.writer(f);writer.writerow(['cell','pairs_scored','pairs_with_native_truncation','fraction_with_native_truncation'])
        for cell in CELLS:
            item=result['coverage'][cell]
            writer.writerow([cell,item['pairs_scored'],item['pairs_with_native_truncation'],item['fraction_with_native_truncation']])
    lines=['# RAPPPID latest recovery checkpoints: C1/C2/C3 test', '',
        f"Completed: {result['at_utc']}. All three user-selected mid-epoch-8 recovery checkpoints; no training restart.",
    ...
    lines.extend(['', 'Primary candidate: FP64 mean of all three native pre-sigmoid logits. All three seed results are also reported, with no seed selection based on test scores.',

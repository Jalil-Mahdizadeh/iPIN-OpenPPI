...
interaction (MI:0407), no negative flag, no modelled or intramolecular flag, no
expansion, matching organism taxids from `config.json`, UniProt identities,
explicit reference sequences, and **no recorded participant features**. This
conservative feature rule excludes annotated mutants/fragments/tags as well as
other feature-bearing records. It does not prove that unannotated experimental
constructs were full-length wild type. Results concern the archived reference
sequence projection of that evidence. Negative records remain separate and

...
def parse_species(species):
    out = LOCAL / (species['id'] + '_parsed.json.gz')
    if out.exists():
            ...
            with z.open(member) as f:
                for event, e in ET.iterparse(f, events=('start', 'end')):
                    ...
                    if tag == 'interactor':
                        ...
                        if v['interaction_type_ac'] != 'MI:0407':
                            reasons.append('not_direct_interaction')
                        ...
                        if features:
                            reasons.append('recorded_participant_features')
                        ...
                        if not reasons:
                            ...
                            positive[pair].append(dict(archive=species['archive'], member=member, entry=entry,
                                ...
                                negative_flag=v['negative'], participant_feature_count=0, interaction_type='MI:0407'))
    ...
    write_json(OUT / (species['id'] + '_SOURCE_AUDIT.json'), summary)
    return summary
...
def main():
    ...
    write_csv(OUT / 'source_feasibility.csv', [{k: s[k] for k in ('species', 'taxid', 'source_accessions',
        'eligible_unique_sequences', 'positive_pairs', 'exclusion_pairs', 'targets_with_at_least_two_P')} for s in summaries])
    write_json(OUT / 'SOURCE_PARSING.json', dict(at_utc=now(), summaries=summaries,

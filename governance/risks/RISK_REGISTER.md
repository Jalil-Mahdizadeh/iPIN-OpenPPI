...
| R-012 | Heterogeneous negative evidence is collapsed into a universal nonbinding class | High | Critical | Production audit keeps ME and SN families separate, preserves context, assigns five evidence tiers, and emits zero universal/label-authorized rows; binding prohibition remains | Codex | Mitigated; controlled |
| R-013 | Historical Negatome stringent filtering is mistaken for current conflict-free evidence | High | High | Production reconciliation found 237 current positive conflicts and preserves CF-D/CF-B overlays rather than deleting rows; stringent status is explicitly historical | Codex | Realized; mitigated |
| R-014 | Negatome redistribution terms are assumed from public download or article license | Medium | High | Raw and record-level data remain internal and out of Git; only hashes, code, schemas, and non-extractive aggregates are releasable pending permission | Codex / sponsor | Open; constrained |
| R-015 | A selected conditional non-detection panel is treated as a representative population-negative sample | High | Critical | Audit preserves all five outcome states and finds only a conditional assay-specific diagnostic feasible; panel remains quarantined, external-only, and governance-pending | Codex | Realized; controlled |
| R-016 | Preview-only IMEx curation is mistaken for a stable current IntAct release | High | High | IM-30553 is frozen as a dated preview with local hashes, kept separate from Release 252, and tracked in ISSUE-0006 | Codex | Realized; controlled |
| R-017 | The paper's 4,100 selected-pair denominator is silently replaced by the 4,130 unique public archive pairs | High | High | Preserve both counts, prohibit arbitrary deletion of 30 pairs, and require author clarification or expert waiver under ISSUE-0007 before integration | Codex / expert group | Open; blocking integration |
| R-018 | IMEx interaction records silently override Data 22 negative or technical outcomes | Medium | High | Preserve both representations; quarantine the two discrepant pairs and require explicit disposition under ISSUE-0008 | Codex / expert group | Open; blocking integration |

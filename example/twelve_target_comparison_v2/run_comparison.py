...
def load_panels(root):
    rows, files, expected = [], [], {}
    for gene in TARGETS:
        folder = Path(__file__).resolve().parent / "panels" / gene
        ...
        for i, (row, annotation) in enumerate(zip(panel, manifest["rows"], strict=True), 1):
            ...
            rows.append({**row, "panel_row": i,
                         ...
                         "original_tuna_prior_pair": any(annotation.get(f"original_tuna_{role}_{kind}_labels", "")
                                                         for role in ("training", "validation") for kind in ("accession", "exact_sequence")),
                         "low_plausibility_evidence_tier": annotation.get("evidence_tier", ""),
    ...
    if len(rows) != 5587 or sum(r["class"] == "P" for r in rows) != 37:
...
def prepare(root, out):
    ...
    original = {}
    # Preserve only completed scientific artifacts, not multi-GB local caches.
    for relative in ("example/twelve_target_comparison_v1/RUN_MANIFEST.json",
                     "example/u_context_background_analysis_v1/FINAL_MANIFEST.json"):
        manifest = read(root / relative)
        original[relative] = sha(root / relative)
        for item in manifest.get("scientific_outputs", manifest.get("outputs", [])):
            path = root / item["path"]
            if sha(path) != item["sha256"]:
                raise RuntimeError(f"Historical artifact changed: {path}")
            original[str(path.relative_to(root))] = item["sha256"]
    # The original four-model application has no completed-run manifest.
    for path in sorted((root / "example/six_target_comparison_v1").iterdir()):
        if path.suffix in (".py", ".json", ".csv", ".md"):
    ...
    plan = {"at_utc": now(), "input_files": files, "original_file_hashes": original,
            "pairs": len(rows), "P": 37, "U": 5550, "accessions": len(accessions),
...
def inputs(root, out):
    ...
    if sha(out / "SELECTION.md") != plan["selection_protocol_sha256"]:
...
def tuna(root, out):
    ...
    original_bundle = root / "benchmark/tuna/runs/scorer_bundle"
    original_freeze = read(original_bundle / "SCORER_FREEZE.json")
    original_item = next(r for r in original_freeze["files"] if r["path"] == "weights/tuna_original.pt")
    original_path = original_bundle / original_item["path"]
    if sha(original_path) != original_item["sha256"] or original_item["sha256"] != "fce619249663193ea17766fd6afa43abcf110a49a5a91d625450cfa966586ae2":
        raise RuntimeError("Original TUnA frozen runtime checkpoint changed")
    verified.append(record(original_path))
    verified.append(record(original_bundle / "SCORER_FREEZE.json"))
    members = ["tuna_original"] + [f"tuna_retrained_seed{s}" for s in SEEDS]
    scores, qualifications = {}, []
    for member in members:
        path = original_path if member == "tuna_original" else bundle / "weights" / f"{member}.pt"

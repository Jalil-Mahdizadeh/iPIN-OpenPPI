...
TARGETS = ("ERN1", "TP53", "EGFR", "BCL2", "KEAP1", "BRCA1")
INPUT_COLUMNS = ("query_gene", "query_uniprot", "partner_gene", "partner_uniprot", "class")
MODELS = ("ipin_baseline", "ipin_optimized", "tuna_original", "tuna_retrained")
...
def load_panels(root):
    ...
    if len(rows) != 1919 or sum(r["class"] == "P" for r in rows) != 19:
...
def prepare(root, out):
    ...
    for path in sorted((root / "example").rglob("*")):
        if path.is_file() and "six_target_comparison_v1" not in path.relative_to(root).parts:
            original[str(path.relative_to(root))] = sha(path)
    for rel in ("graphify-out/graph.json", "graphify-out/cache/last_query_stamp"):
        if (root / rel).is_file():
    ...
    plan = {"at_utc": now(), "input_files": files, "original_file_hashes": original,
            "pairs": len(rows), "P": 19, "U": 1900, "accessions": len(accessions),
            ...
            "metrics_prespecified": "Unweighted P-versus-U concordance with exact half-ties; per target and U stratum; matched-anchor P-vs-U; tie-aware positive ranks and top-10 recovery; equal-target macro means; sensitivity excluding development-exposed P and separately homomers.",
...
def tuna(root, out):
    ...
    rows, snapshot, order, sequences = inputs(root, out)
    scripts = root / "benchmark/tuna/scripts"
    sys.path.insert(0, str(scripts))
    import adapter
    import esm_cache
    bundle = root / "benchmark/tuna/runs/scorer_bundle"
    ...
    # Six first nominated pairs (including ERN1 self-pair) plus the longest pair.
    ...
    members = ["tuna_original"] + [f"tuna_retrained_seed{s}" for s in SEEDS]
    ...
    for member in members:
        ...
        score = adapter.cached_scores(model, z, a, b, probabilities=member == "tuna_original")
        reverse = adapter.cached_scores(model, z, b, a, probabilities=member == "tuna_original")
        ...
        torch.cuda.empty_cache()
    scores["tuna_retrained"] = np.column_stack([scores[m] for m in members[1:]]).mean(1, dtype=np.float64)
    ...
    write_json(out / "TUNA_RUN.json", {"at_utc": now(), "elapsed_seconds": time.monotonic() - started,
        ...
        "verified_model_inputs": verified, "scorer_freeze": record(bundle / "SCORER_FREEZE.json"),
...
def concordance(positive_scores, unlabeled_scores):
    import numpy as np
    p, u = np.asarray(positive_scores, np.float64), np.asarray(unlabeled_scores, np.float64)
    if not len(p) or not len(u) or not np.isfinite(p).all() or not np.isfinite(u).all():
        raise ValueError("Concordance requires finite P and U scores")
    return float(((p[:, None] > u).astype(float) + .5 * (p[:, None] == u)).mean())


def ranks(score, population, k=10):
    greater = sum(v > score for v in population)
    tied = sum(v == score for v in population)
    if not tied:
        raise ValueError("Score not in population")
    return greater + 1, greater + tied, greater + (tied + 1) / 2, min(1., max(0., (k - greater) / tied))
...
def summarize(root, out):
    ...
    for row in rows:
        if np.mean([row[c] for c in member_cols], dtype=np.float64) != row["tuna_retrained_score"]:
            raise RuntimeError("TUnA ensemble does not equal all three fixed seeds")
    metrics, positive_ranks = [], []
    for gene in TARGETS:
        panel = [r for r in rows if r["query_gene"] == gene]
        p = [r for r in panel if r["class"] == "P"]
        u = [r for r in panel if r["class"] == "U"]
        for model in MODELS:
            key = model + "_score"
            population = [r[key] for r in panel]
            for row in panel:
                row[model + "_rank"] = ranks(row[key], population)[2]
            for positive in p:
                low, high, mid, top10 = ranks(positive[key], population)
                own = [r for r in u if r["anchor_positive_uniprot"] == positive["partner_uniprot"]]
                positive_ranks.append({"query_gene": gene, "partner_gene": positive["partner_gene"],
                    "partner_uniprot": positive["partner_uniprot"], "model": model, "score": positive[key],
                    "rank_min": low, "rank_max": high, "rank_mid": mid, "panel_size": len(panel),
                    "top10_fractional_credit": top10, "fraction_U_outranked_half_ties": concordance([positive[key]], [r[key] for r in u]),
                    "matched_100U_concordance": concordance([positive[key]], [r[key] for r in own]),
                    "matched_context_50U_concordance": concordance([positive[key]], [r[key] for r in own if r["U_stratum"] == "context"]),
                    "matched_background_50U_concordance": concordance([positive[key]], [r[key] for r in own if r["U_stratum"] == "background"]),
                    "development_exposed_positive": positive["development_exposed_positive"], "homomeric": positive["homomeric"]})
            for subset in ("all_P", "exclude_development_P", "exclude_development_P_and_homomers"):
                pp = [r for r in p if subset == "all_P" or (not r["development_exposed_positive"] and (subset != "exclude_development_P_and_homomers" or not r["homomeric"]))]
                for stratum in ("all_U", "context", "background"):
                    uu = [r for r in u if stratum == "all_U" or r["U_stratum"] == stratum]
                    metrics.append({"target": gene, "model": model, "positive_subset": subset, "U_stratum": stratum,
                        "P": len(pp), "U": len(uu), "P_vs_U_concordance": concordance([r[key] for r in pp], [r[key] for r in uu])})
        write_csv(out / f"{gene.lower()}_four_model_scores.csv", panel)
    write_csv(out / "all_six_targets_scores.csv", rows)
    write_csv(out / "per_target_metrics.csv", metrics)
    write_csv(out / "positive_partner_ranks.csv", positive_ranks)
    summary = []
    for subset in ("all_P", "exclude_development_P", "exclude_development_P_and_homomers"):
        for model in MODELS:
            scores = [r["P_vs_U_concordance"] for r in metrics if r["positive_subset"] == subset and r["U_stratum"] == "all_U" and r["model"] == model]
            summary.append({"model": model, "positive_subset": subset, "targets": 6,
                            "macro_P_vs_U_concordance": float(np.mean(scores))})
    write_csv(out / "macro_metrics.csv", summary)
    for gene in TARGETS:
        source = out / f"{gene.lower()}_four_model_scores.csv"
        target = root / "example" / gene / source.name
    ...
    write_json(out / "VALIDATION.json", {"at_utc": now(), "passed": True,
        "pairs": len(rows), "four_model_scores": len(rows) * 4, "P": 19, "U": 1900,
    ...
    print(json.dumps({"macro_metrics": summary, "validation": "passed"}, indent=2), flush=True)

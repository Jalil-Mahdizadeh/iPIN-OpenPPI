#!/usr/bin/env python3
"""Standalone validation of the corrected frozen development evaluation.

Version 2 preserves the historical standalone metric and score formulas, but
joins pair identities to manifest storage rows independently. Production moves
matrix rows; this validator leaves matrices in storage order and moves indices.
The v1 validator and its evidence remain unchanged.
...
PRODUCTION_EVIDENCE_COMMIT = "1bae702a98c488201b7c7078f3b665bcc026db90"
PRODUCTION_REGISTRY_SHA256 = (
    "58ca277f6bed2b248de482a6b510091872e9d2b5d55dd916d611ca943b7752cb"
)
PRODUCTION_AUDIT_SHA256 = (
    "3d448271777a2d9060bdeb4f56142d29274c6ea7eecc267a6281c454c829b081"
)
TRAINING_REGISTRY_SHA256 = (
    "11d7a92d6dd42ca78434783844cbba2ffb05ac789b76eca4399528d0d19ab318"
)
SCORING_MANIFEST_SHA256 = (
    "7348e2044260b2ce9d75281805e4101356d2d2d34cd8f7c5b531f7a4b010917d"
)
RESULTS_MANIFEST_SHA256 = (
    "012ff5d1fc4a79d666a65db9a16a7df37d155eb292675f275d4af1ef49a6e811"
)
SELECTION_TRACE_SHA256 = (
    "3cc9eb411697ec034e656ab84534d4ca47414a55461c66e735b48e3d71fa47ea"
)
CONFIG_SHA256 = "45feca41cedc0069f8a9ef499bb9136040dedf3dd851cbff62a2c2ebefc061b8"

VALIDATION_ROOT = Path(
    "artifacts/validation/development_evaluation/"
    "development_embedding_identity_correction_v2"
)
RESULTS_ROOT = Path(
...
PRIVATE_EVALUATION_ROOT = Path(
    ".private/development_embedding_identity_correction_v2/evaluation"
)
...
SOURCE_HASHES = {
    "scripts/model/audit_development_completed_v1.py": "18cb40412176bdc412d224f6b5f91ca1a25281aabea3672cf2dccfbee399afd1",
    "scripts/model/audit_development_embedding_identity_preflight_v2.py": "2141313873ca4c2d0c097929b92ab8e441710b155f753e91872406be1ed1e4f4",
    "scripts/model/evaluate_development_v1.py": "899d14c05a179fe60fb69ac8f7473519686c9b1e6f1324f781baca50a370bade",
    "scripts/model/run_development_scoring_v1.py": "6b202128fe5e2b453a913b43bb103bdbf91aa76ff2c07d98ce16aef3bf78db42",
    "src/ipin_openppi/development_evaluation/completed_audit.py": "693f8b9614b31c6fe02d1e4aa81c2a47414c54810dd9354ef19f0ea60388c0aa",
    "src/ipin_openppi/development_evaluation/embedding_identity.py": "09ab9a4783edf21813a1ed5be47cf1548869e47dd957050549e78556241ff12e",
    "src/ipin_openppi/development_evaluation/evaluation.py": "df8b4949c3e94120a816ea981ad99dd4138826cd896e33153980ea4763fec38f",
    "src/ipin_openppi/development_evaluation/scoring.py": "ef8d114ad88d69cd5366746d3cb90c9df3f26c1e481111c317de092651226da0",
    "src/ipin_openppi/development_evaluation/semantics.py": "5e63276dbb769659dcb3ca636f0022c485a05bd82f5fc6855ee6aa5b2ee7bd00"
}

IDENTITY_PREFLIGHT_SHA256 = "c8b4a10c960681c56f8a0de633f10eaa47ba7c5389b4b3c8cc695ea59af0d4de"
EMBEDDING_MANIFEST_HASHES = {
    "esm2_150m": "bc4ea08691cdd55f393fc6ebb6264c338c70846b82e5a9001c3ac2a57a34d27f",
    "esm2_650m": "878bd90d60dcfcf546479c94255e673ebd18f10756179cfcafac680028a53902",
}
...
def _safe_regular(project_root: Path, relative: Path) -> Path:
    ...
    if ".private" in relative.parts:
        allowed_prefix = (".private", "development_embedding_identity_correction_v2")
...
def _check(
    ...
    print(f"independent check: {check_id} {'PASS' if condition else 'FAIL'}", flush=True)
...
def _storage_rows_from_manifest(
    endpoint_sha: Sequence[str],
    lengths: Sequence[int],
    manifest: Mapping[str, Any],
    matrix_shape: tuple[int, int],
    candidate: str,
) -> np.ndarray:
    """Resolve each endpoint coordinate to a verified matrix storage coordinate."""
    count, dimension = matrix_shape
    identities = tuple(endpoint_sha)
    if (
        len(identities) != count
        or len(set(identities)) != count
        or len(lengths) != count
        or manifest["candidate_id"] != candidate
        or manifest["vector_count"] != count
        or len(manifest["vectors"]) != count
    ):
        raise RuntimeError("independent identity universe/candidate census mismatch")
    expected_lengths = dict(zip(identities, map(int, lengths), strict=True))
    records_by_row: list[Mapping[str, Any] | None] = [None] * count
    for record in manifest["vectors"]:
        row = record["row_index"]
        identity = record["sequence_sha256"]
        if (
            type(row) is not int
            or row < 0
            or row >= count
            ...
            or identity not in expected_lengths
            or record["sequence_length"] != expected_lengths[identity]
            or record["candidate_id"] != candidate
            or record["vector_dimension"] != dimension
        ):
            raise RuntimeError("independent manifest identity/row/vector mismatch")
        if records_by_row[row] is not None:
            raise RuntimeError("independent duplicate embedding row")
        records_by_row[row] = record
    if any(record is None for record in records_by_row):
        raise RuntimeError("independent incomplete embedding row coverage")
    # Enumerate physical rows, never infer coordinates from the manifest list.
    physical_by_identity = {
        str(record["sequence_sha256"]): row
        for row, record in enumerate(records_by_row)
        if record is not None
    }
    if set(physical_by_identity) != set(identities):
        raise RuntimeError("independent duplicate or missing embedding identity")
    return np.fromiter(
        (physical_by_identity[identity] for identity in identities),
        dtype=np.int64,
        count=count,
    )


def _verify_embedding_identity(
    project_root: Path,
    candidate: str,
    relative: Path,
    expected_hash: str,
    ...
    manifest_path = _safe_regular(
        project_root, relative.with_name("EMBEDDING_MANIFEST.json")
    )
    if _sha256(manifest_path) != EMBEDDING_MANIFEST_HASHES[candidate]:
        raise RuntimeError(f"independent frozen manifest hash drift: {candidate}")
    manifest = _json(manifest_path)
    rows = _storage_rows_from_manifest(
        endpoint_sha, lengths, manifest, matrix.shape, candidate
    )
    normalization = manifest["normalization"]
    if (
        normalization["standardized_matrix_path"] != relative.as_posix()
        or normalization["standardized_matrix_sha256"] != expected_hash
        or not np.isfinite(matrix).all()
    ):
        raise RuntimeError(f"independent matrix/manifest binding drift: {candidate}")
    raw_path = _safe_regular(project_root, Path(manifest["matrix_path"]))
    normalizer_path = _safe_regular(project_root, Path(normalization["normalizer_path"]))
    if (
        _sha256(raw_path) != manifest["matrix_sha256"]
        or _sha256(normalizer_path) != normalization["normalizer_sha256"]
    ):
        raise RuntimeError(f"independent raw/normalizer checksum drift: {candidate}")
    raw = np.load(raw_path, mmap_mode="r", allow_pickle=False)
    if raw.shape != matrix.shape or raw.dtype != np.float32:
        raise RuntimeError(f"independent raw embedding shape/dtype drift: {candidate}")
    for record in manifest["vectors"]:
        if hashlib.sha256(raw[record["row_index"]].tobytes()).hexdigest() != record["vector_sha256"]:
            raise RuntimeError(f"independent vector identity hash drift: {candidate}")
    with np.load(normalizer_path, allow_pickle=False) as normalizer:
        mean = normalizer["mean"]
        deviation = normalizer["standard_deviation"]
        if (
            mean.shape != (matrix.shape[1],)
            or deviation.shape != mean.shape
            or not np.isfinite(mean).all()
            or not np.isfinite(deviation).all()
            or np.any(deviation <= 0)
        ):
            raise RuntimeError(f"independent normalizer value drift: {candidate}")
        for start in range(0, matrix.shape[0], 256):
            stop = start + 256
            recovered = ((raw[start:stop].astype(np.float64) - mean) / deviation).astype(np.float32)
            if not np.array_equal(recovered, matrix[start:stop]):
                raise RuntimeError(f"independent normalization reconstruction drift: {candidate}")
    # Hash endpoint-ordered bytes one physical row at a time; inference below
    # continues to use the untouched storage-order matrix.
    aligned_digest = hashlib.sha256()
    for row in rows:
        aligned_digest.update(matrix[row].tobytes())
    record = {
        "method": "translate_pair_endpoint_coordinates_to_verified_storage_rows",
        "manifest_sha256": EMBEDDING_MANIFEST_HASHES[candidate],
        ...
        "endpoint_order_sha256": hashlib.sha256("\n".join(endpoint_sha).encode("ascii")).hexdigest(),
        "aligned_matrix_sha256": aligned_digest.hexdigest(),
        "all_raw_vector_hashes_verified": True,
        "all_standardized_values_reconstructed_exactly": True,
        "inference_matrix_in_original_storage_order": True,
    }
    return rows, record
...
def _model_score_validation(
    ...
    endpoint_sha: Sequence[str],
    lengths: Sequence[int],
) -> tuple[bool, float, float, int, dict[str, Any]]:
    embedding_tensors: dict[str, torch.Tensor] = {}
    storage_rows: dict[str, np.ndarray] = {}
    ...
    for candidate, (relative, expected_hash, expected_shape) in EMBEDDINGS.items():
        ...
        storage_rows[candidate], identity_records[candidate] = _verify_embedding_identity(
            project_root, candidate, relative, expected_hash, matrix, endpoint_sha, lengths
        )
        print(f"independent embedding identity: {candidate} rows={matrix.shape[0]}", flush=True)
        ...
        with torch.inference_mode():
            ...
            for cell in cell_views:
                for start in range(0, cell.endpoint_a.size, 32_768):
                    ...
                    a = torch.from_numpy(
                        np.asarray(storage_rows[candidate][cell.endpoint_a[start:stop]], dtype=np.int64)
                    ).cuda()
                    b = torch.from_numpy(
                        np.asarray(storage_rows[candidate][cell.endpoint_b[start:stop]], dtype=np.int64)
                    ).cuda()
                ...
                fixture_a = torch.from_numpy(
                    np.asarray(storage_rows[candidate][cell.endpoint_a[:fixture_stop]], dtype=np.int64)
                ).cuda()
                fixture_b = torch.from_numpy(
                    np.asarray(storage_rows[candidate][cell.endpoint_b[:fixture_stop]], dtype=np.int64)
                ).cuda()
    ...
    torch.cuda.empty_cache()
    return exact, maximum, swap_maximum, comparisons, identity_records
...
def validate(
    ...
    config_path = _safe_regular(
        project_root, Path("configs/development_embedding_identity_correction_v2.yaml")
    )
    ...
    evidence_ok = (
        ...
        == "5548baa20ae832abfc7f7eae41ecebd261f38c54"
        and scoring_manifest["cell_count"] == 9
        and scoring_manifest["scorer_count"] == 49
        and results_manifest["stop_before_protected_evaluation"]
        == registry["stop_before_protected_evaluation"]
        and registry["protected_evaluation_authorized"] is False
    )
    _check(
        checks,
    ...
    preflight_path = _safe_regular(
        project_root, VALIDATION_ROOT / "IDENTITY_PREFLIGHT_REPORT.json"
    )
    preflight = _json(preflight_path)
    source_ok = (
        _sha256(preflight_path) == IDENTITY_PREFLIGHT_SHA256
        and preflight["status"] == "pass"
        and preflight["source_hashes"] == SOURCE_HASHES
        and preflight["execution_config_sha256"] == CONFIG_SHA256
        and scoring_manifest["identity_preflight_sha256"] == IDENTITY_PREFLIGHT_SHA256
        and scoring_manifest["source_hashes"] == SOURCE_HASHES
    )
    ...
    registered_ok = len(registered) == 56
    ...
    _check(
        ...
        registered_ok and len(unique_records) == 56,
        {
    ...
    if any(
        hashlib.sha256(sequence.encode("ascii")).hexdigest() != identity
        or len(sequence) != int(length)
        for identity, sequence, length in zip(endpoint_sha, sequences, lengths, strict=True)
    ):
        raise RuntimeError("independent public sequence identity/length mismatch")
    ...
    model_exact, model_maximum, swap_maximum, model_comparisons, identity_records = (
        _model_score_validation(project_root, cell_views, training_registry, endpoint_sha, lengths)
    )
    identity_keys = (
        "endpoint_count", "reordered_row_count", "embedding_row_permutation_sha256",
        "endpoint_order_sha256", "aligned_matrix_sha256",
    )
    identity_ok = all(
        cell["embedding_identity"][candidate][key] == identity_records[candidate][key]
        for cell in scoring_manifest["cells"]
        for candidate in EMBEDDINGS
        for key in identity_keys
    )
    _check(
        checks,
        "independent_manifest_vector_identity_and_storage_coordinate_translation",
        identity_ok,
        identity_records,
    )
    _check(
    ...
    disposition_ok = (
        ...
        == results_manifest["development_stage_disposition"]
        and disposition["kill_trace"]["stop_before_protected_evaluation"]
        == results_manifest["stop_before_protected_evaluation"]
    )
    _check(
        checks,
    ...
    report = {
        "schema_version": 2,
        "execution_id": "development_embedding_identity_correction_v2",
        ...
        "stop_before_protected_evaluation":
            disposition["kill_trace"]["stop_before_protected_evaluation"],
        "protected_evaluation_authorized": False,
        "embedding_identity_validation": identity_records,

...
if [[ -z "${SLURM_JOB_ID:-}" || -z "${CUDA_VISIBLE_DEVICES:-}" ]]; then
    echo "A four-GPU Slurm allocation is required." >&2
    exit 2
fi
...
run_id="platform-four-gpu-long-${SLURM_JOB_ID}-${run_timestamp}"
...
apptainer exec --nv --cleanenv --no-home \
    ...
    --run-label single_gpu_long_baseline \
    --image-sha256 "${image_sha256}" \
    --warmup-steps 50 \
    --measured-steps 1000 2>&1 | tee "${run_dir}/single_gpu.log"
...
apptainer exec --nv --cleanenv --no-home \
    ...
    --project-root "${IPIN_PROJECT_ROOT}" \
    --run-label four_gpu_ddp_long \
    --image-sha256 "${image_sha256}" \
    --warmup-steps 50 \
    --measured-steps 1000 2>&1 | tee "${run_dir}/four_gpu.log"
...
echo "Extended four-GPU qualification completed: ${run_dir}"
